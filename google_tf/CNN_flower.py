...
# 数据路径
DATA_PATH = '../data'
MODEL_DIR = DATA_PATH + '/inception_dec_2015'
MODEL_FILE = 'tensorflow_inception_graph.pb'
CACHE_DIR = DATA_PATH + '/temp'
INPUT_DATA = DATA_PATH + '/flower_photos'
...
def get_random_cached_bottleneck(
        sess, n_classes, image_lists, how_many, category, jpeg_data_tensor,
        bottleneck_tensor):
    bottlenecks = []
    ground_truths = []
...
def get_tst_bottlenecks(sess, image_lists, n_classes, jpeg_data_tensor,
                        bottleneck_tensor):
    bottlenecks = []
    ground_truths = []
    ...
    for label_index, label_name in enumerate(label_name_list):
        ...
        for index, unused_base_name in enumerate(image_lists[label_name][category]):
            bottleneck = get_or_create_bottleneck(
                sess, image_lists, label_name, index, category, jpeg_data_tensor,
                bottleneck_tensor)
            ground_truth = np.zeros(n_classes, dtype=np.float32)
            ground_truth[label_index] = 1.0
            bottlenecks.append(bottleneck)
    ...
    with tf.Session() as sess:
        ...
        for i in range(STEPS):
            ...
            if i % 100 == 0 or i + 1 == STEPS:
                ...
                print("%d: Validation accuracy %d examples = %.1f%%" %
                      (i, BATCH, validation_accuracy * 100))
        test_bottlenecks, test_ground_truch = get_tst_bottlenecks(
            sess, image_lists, n_classes, jpeg_data_tensor, bottleneck_tensor)
        test_accuracy = sess.run(evaluation_step, feed_dict={
            bottleneck_input: test_bottlenecks,
            ground_truth_input: test_ground_truch})
